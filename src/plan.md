...
- [ ] Phase 6: Tích hợp OCR (Tesseract) và Hiển thị kết quả.
...
*   **Phase 6: Tích hợp OCR (Tesseract) & Overlay**
    - [ ] Cài đặt NuGet: `Tesseract` và `Tesseract.Drawing`.
    - [ ] Cấu hình `AllowUnsafeBlocks` trong project file để tối ưu tốc độ xử lý ảnh.
    - [ ] Tạo model `OcrResult` để lưu trữ Text, Bounding Box (Rect) và Confidence.
    - [ ] Triển khai `InitializeOcr()`: Load `tessdata` (eng/vie) từ thư mục thực thi.
    - [ ] Viết hàm `PerformOcr()`:
    ...
    - [ ] Hiển thị kết quả:
        - Sử dụng `ItemsControl` bound tới `OcrResults` để vẽ các khung đỏ (`Border`) lên trên ảnh.
        - Thêm Tooltip hiển thị nội dung Text khi hover chuột.